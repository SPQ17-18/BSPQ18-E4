...
public class MyAccount extends JFrame {
	...
	private void initialize() {
		...
		noReserv = new JLabel("");
		noReserv.setForeground(Color.RED);
		noReserv.setFont(new Font("Tahoma", Font.PLAIN, 24));
		noReserv.setHorizontalAlignment(SwingConstants.CENTER);
		noReserv.setBounds(10, 11, 414, 146);
		frame.getContentPane().add(noReserv);
		addDataByUser(userDTO);
		if (table.getRowCount()>0) {
			noReserv.setVisible(false);
		} else {
			noReserv.setText("There are no reservations");
			noReserv.setVisible(true);
		}
	}
	...
	private void addDataByUser(UserDTO userDTO) {
		...
		for (Reservation reservation : reservations) {
			//System.out.println(reservation.getArrival()+"     "+ reservation.getDeparture()+"hot  "+reservation.getHotel());
			model.addRow(new Object[] { reservation.getArrival(), reservation.getDeparture() });
		}
